...
class MainWindow:
    ...
    def view_notes(self):
        ...
        self.view_button = tk.Button(
            self.notes_window, text="Просиотреть", command=self.view_note
        )
        self.view_button.pack(side=tk.LEFT, padx=10)

        self.edit_button = tk.Button(
            self.notes_window, text="Редактировать", command=self.edit_note
        )
        self.edit_button.pack(side=tk.LEFT, padx=10)

        self.delete_button = tk.Button(
            self.notes_window, text="Удалить", command=self.delete_note
        )
    ...
    def edit_note(self):
        ...
        self.save_button = tk.Button(
            self.edit_window,
            text="Сохранить",
            command=lambda: self.save_edited(selected_note),
        )
        self.save_button.pack(side=tk.LEFT, padx=10)

        self.cancel_button = tk.Button(
            self.edit_window, text="Отменить", command=self.edit_window.destroy
        )
        self.cancel_button.pack(side=tk.RIGHT, padx=10)
